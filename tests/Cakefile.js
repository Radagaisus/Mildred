...
(function() {
  ...
  task('build', 'Build single test file from source files', function() {
    ...
    return process = function() {
      return fs.writeFile('tests.coffee', appContents.join('\n\n'), 'utf8', function(err) {
        if (err) {
          throw err;
        }
        return exec('coffee --compile tests.coffee', function(err, stdout, stderr) {
          if (err) {
            throw err;
          }
          console.log(stdout + stderr);
          return fs.unlink('tests.coffee', function(err) {
            if (err) {
              throw err;
            }
